...
namespace Ruya.AppDomain
{
    ...
    public class UnhandledExceptionHelper
    {
        ...
        public void SetLogger(ILoggerFactory logger) => _logger = logger.CreateLogger(nameof(UnhandledExceptionHelper));

        public System.AppDomain ApplicationDomain { get; }

        public void Register(bool ignoreDebugger = false)
        {
            if (ignoreDebugger || !Debugger.IsAttached)
            {
        ...
        public void LogExistingCrashes(bool deleteLogFile)
        {
            ...
            string[] files = Directory.GetFiles(Directory.GetCurrentDirectory(), $"{nameof(UnhandledExceptionEventHandler)}{Constants.WildCardStar}{FileExtension}", SearchOption.TopDirectoryOnly);
            if (!files.Any())
            {
                _logger.LogDebug("No UnhandledException file found.");
            }
            for (int index = 0;
                 index < files.Length;
                 index++)
            {
                ...
                string suffix = Path.GetFileNameWithoutExtension(file)
                                      ...
                                      .TrimStart();
                if (DateTimeOffset.TryParseExact(suffix, Constants.FileSystemSafeDateTimeOffset, CultureInfo.InvariantCulture, DateTimeStyles.None, out DateTimeOffset dateTimeOffset))

                    _logger.LogWarning($"Unhandled exception file/s detected. [{index / files.Length}] {{datetime}} {{content}}", dateTimeOffset, content);
                if (deleteLogFile)
                {
                    File.Delete(file);
                }
        ...
        public void Unregister() => ApplicationDomain.UnhandledException -= UnhandledExceptionHandler;
        ...
        private void UnhandledExceptionHandler(object sender, UnhandledExceptionEventArgs args)
        {
            ...
            string path = $"{nameof(UnhandledExceptionEventHandler)} {DateTimeOffset.UtcNow.ToString(Constants.FileSystemSafeDateTimeOffset)}{FileExtension}";
            try
            {
