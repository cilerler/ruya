...
namespace Ruya.Bus.RabbitMQ
{
	...
	public sealed class EventBusRabbitMq : IEventBus, IDisposable
	{
		...
		private Dictionary<string, IModel> _consumerChannels = new Dictionary<string, IModel>();
		...
		{
			_logger.LogDebug("BasicAcks message has been received by RabbitMQ.  Session {@Sender} BasicAckEventArgs {@BasicAckEventArgs}", sender.ToString(),e);
		}
		...
		public void AddConsumerChannel(string queueName)
		{
			...
			_consumerChannels.Add($"{queueName}_{Guid.NewGuid()}"
			                    , CreateConsumerChannel(queueName));
		}
		...
		private IModel CreateConsumerChannel(string queueName)
		{
			...
			consumer.Received += async (object sender, BasicDeliverEventArgs basicDeliverEventArgs) =>
			                     {
				                     ...
				                     _logger.LogDebug("BasicDelivery message has been received from RabbitMQ. BasicDeliverEventArgs {ReDelivered} {EventName} {@Parameters} {Message}"
			...
			channel.BasicConsume(queue: queueName //_options.SubscriptionClientName
			                   , autoAck: false
			                   , consumer: consumer);
		...
		private async Task ProcessEvent(string eventName, string message, Dictionary<string, object> parameters)
		{
				...
				using (IServiceScope scope = scopeFactory.CreateScope())
				{
					...
					foreach (SubscriptionInfo subscription in subscriptions)
					{
						...
						else
						{
							...
							object integrationEvent = JsonConvert.DeserializeObject(message
							                                                      , eventType);
							Type concreteType = typeof(IIntegrationEventHandler<>).MakeGenericType(eventType);
							if (!concreteType.IsInstanceOfType(handler))
							{
								_logger.LogWarning("Handler doesn't implement `IIntegrationEventHandler<>` EventName {eventName} Message {message} Subscription {@subscription}", eventName, message, subscription);
			...
			{
				_logger.LogWarning("Received a payload for event {EventName} but there is no subscription exists. The payload is {Message}", eventName, message);
