--! Ruya MessageQueue - Resolve and provision Service Broker topology for a logical topic.
--! Parameters:
...
--! Security: The stored procedure validates ownership and quotes dynamic identifiers.
SET NOCOUNT ON;

/*
-- DEBUG: Uncomment this block to test the script in SSMS.
DECLARE @p0 NVARCHAR(255) = N'orders.created';
DECLARE @p1 SYSNAME = N'RuyaServicesMessageQueueQueue_orders';
DECLARE @p2 SYSNAME = N'RuyaServicesMessageQueueService_orders';
...
IF NULLIF(@TopicName, N'') IS NULL
    THROW 51100, 'TopicName is required.', 1;
IF NULLIF(@ProposedQueueName, N'') IS NULL OR NULLIF(@ProposedServiceName, N'') IS NULL
    THROW 51101, 'Proposed queue and service names are required.', 1;

IF @Debug = 1
BEGIN
    PRINT N'Ruya MessageQueue would resolve and provision Service Broker topic topology.';
    RETURN;
END;

IF OBJECT_ID(N'dbo.RuyaServicesMessageQueue_CreateTopicService', N'P') IS NULL
    THROW 51111, 'The Ruya topic-provisioning procedure is not installed.', 1;
