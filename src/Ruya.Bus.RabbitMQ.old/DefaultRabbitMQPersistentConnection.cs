...
public class DefaultRabbitMqPersistentConnection : IRabbitMQPersistentConnection
{
	...
	public DefaultRabbitMqPersistentConnection(ILogger<DefaultRabbitMqPersistentConnection> logger, IOptions<BusSetting> options)
	{
		...
		_connectionFactory = new ConnectionFactory
		{
			...
			ClientProperties = new Dictionary<string, object> { { "Product", _options.AppId } }
		};
	...
	{
		if (!_options.EnableDeclarations) return;
		...
		using (IModel channel = CreateModel())
		{
			...
			foreach (Queue queue in _options.Queues)
			{
				...
				if (dlExists)
				{
					...
					channel.QueueDeclare(dlq
						, true
						, false
						, false);

					channel.QueueBind(dlq
						, dlx
						, dlk);
				}
	...
	protected virtual void Dispose(bool disposing)
	{
		try
		{
			...
			_logger.LogDebug("Disposing native resources");
