...
public class Client : ICloudFileService
{
	...
	public ICloudFileMetadata GetFileMetadata(string fileName, string bucketName = "")
	{
		...
		{
			_logger.LogError(fnfe, "File Not Found - {{fileName}} in bucket {{bucketName}}", fileName, _bucketName);
			throw;
		}
	}
		...
		string bucketName = "")
	{
		...
		try
		{
			if (!Directory.Exists(RootPath)) throw new DirectoryNotFoundException($"{RootPath} does not exists.");
	...
	public void DeleteFile(string fileName, string bucketName = "")
	{
		...
		{
			_logger.LogWarning("File Not Found - {{fileName}} in bucket {{bucketName}}", fileName, _bucketName);
			throw;
		}
	}
	...
	public void CopyFile(string sourceBucketName, string sourceFileName, string destinationBucketName, string destinationFileName)
	{
		...
		try
		{
			if (!Directory.Exists(sourceBucket)) throw new DirectoryNotFoundException($"{sourceBucket} does not exists.");
			if (!Directory.Exists(destinationBucket)) throw new DirectoryNotFoundException($"{destinationBucket} does not exists.");
	...
	{
		return fullName.Replace(RootPath, string.Empty).TrimStart(Path.DirectorySeparatorChar);
	}
	...
	private void EnsureEmptyDirectoriesDeleted(string filePath)
	{
		...
		do
		{
			...
			if (pathExists) directoryPath = directoryPath.Substring(0, index);
		} while (!entriesExist
