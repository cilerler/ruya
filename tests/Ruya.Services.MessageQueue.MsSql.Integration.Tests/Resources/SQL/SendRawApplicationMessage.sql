--! Ruya MessageQueue tests - Send a raw serialized application envelope to test topology.
--! Parameters:
--!   @p0 (SYSNAME) - Resolved Service Broker service identifier.
--!   @p1 (VARBINARY(MAX)) - Serialized test envelope bytes.
--!   @p2 (UNIQUEIDENTIFIER OUTPUT) - Created conversation handle.
--!   @p3 (BIT) - Debug mode (1 prints the operation without sending; 0 executes it).
--! Security: The production procedure validates the service; payload bytes are never printed.
SET NOCOUNT ON;
SET XACT_ABORT ON;

/*
-- DEBUG: Uncomment this block to test the script in SSMS.
DECLARE @p0 SYSNAME = N'RuyaServicesMessageQueueService_test';
DECLARE @p1 VARBINARY(MAX) = 0x00;
DECLARE @p2 UNIQUEIDENTIFIER;
DECLARE @p3 BIT = 1;
*/

DECLARE @ServiceName SYSNAME = @p0;
DECLARE @Payload VARBINARY(MAX) = @p1;
DECLARE @ConversationHandle UNIQUEIDENTIFIER;
DECLARE @Debug BIT = COALESCE(@p3, 0);

IF NULLIF(@ServiceName, N'') IS NULL
    THROW 51206, 'ServiceName is required.', 1;
IF @Payload IS NULL
    THROW 51207, 'Payload is required.', 1;

IF @Debug = 1
BEGIN
    PRINT N'Ruya MessageQueue tests would send one raw application message.';
    RETURN;
END;

IF OBJECT_ID(N'dbo.RuyaServicesMessageQueue_SendMessage', N'P') IS NULL
    THROW 51213, 'The Ruya send procedure is not installed.', 1;

BEGIN TRY
    BEGIN TRANSACTION;
    EXEC [dbo].[RuyaServicesMessageQueue_SendMessage]
        @ServiceName = @ServiceName,
        @Payload = @Payload,
        @ConversationHandle = @ConversationHandle OUTPUT;

    SET @p2 = @ConversationHandle;
    COMMIT TRANSACTION;
END TRY
BEGIN CATCH
    IF @@TRANCOUNT > 0
        ROLLBACK TRANSACTION;
    THROW;
END CATCH;
